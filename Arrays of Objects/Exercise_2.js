...
let destinationNamesWithin500Kms = travelDestinations.filter((el) => el.distanceKms <= 500).map((el) => el.destinationName);

let destinationNameReachableByFerry = 
travelDestinations.filter(destination =>{
    if(destination.transportations.includes("ferry")) return destination.destinationName
    }).map((destination => {
...
travelDestinations.filter((destination) => 
{if(destination.distanceKms >= 300 && destination.transportations.includes("train")) return destination.destinationName}).map((destination => {return destination.destinationName}))//
